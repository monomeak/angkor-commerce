...
public class JwtTokenProvider {
    ...
    public JwtTokenProvider(
        @Value("${acme.jwt.secret}") String secret,
        @Value("${acme.jwt.access-token-ttl-minutes}") long accessTokenTtlMinutes
    ) {
    ...
    private static SecretKey resolveKey(String secret) {
        ...
        if (bytes.length < 32) {
            throw new IllegalStateException("acme.jwt.secret must be at least 32 bytes long for HS256");
        }
