/**
 * core-api stores images as MinIO object keys. Product responses hand back the raw
 * key, so build the public URL here — while still passing through anything that
 * already is one, in case the API starts resolving keys itself (as the staff
 * UserMapper does).
 *
