...
-- Separate from `customers` on purpose: a customer row can exist without ever
-- getting storefront login access (e.g. a walk-in/invoice-only customer a
-- staff member entered), and one customer has at most one login.
CREATE TABLE customer_accounts (
    ...
    CONSTRAINT uq_customer_accounts_customer_id
        UNIQUE (customer_id),

    CONSTRAINT chk_customer_accounts_record_status
        CHECK (record_status IN ('ACTIVE', 'INACTIVE', 'DELETED'))
);
...
CREATE INDEX idx_customer_accounts_record_status
    ON customer_accounts (record_status);

CREATE TRIGGER trg_customer_accounts_set_updated_at
BEFORE UPDATE ON customer_accounts
FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

COMMENT ON TABLE customer_accounts IS
    'Self-service storefront login credentials for a customer, one-to-one with customers.';

COMMENT ON COLUMN customer_accounts.password_hash IS
    'Password hash only; never return this field through API responses.';
